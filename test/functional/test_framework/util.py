...
def assert_fee_amount(fee, tx_size, feerate_BTC_kvB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s BRN too low! (Should be %s BRN)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_BTC_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s BRN too high! (Should be %s BRN)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting BRN values"""
...
def get_fee(tx_size, feerate_btc_kvb):
    """Calculate the fee in BRN given a feerate is BRN/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_btc_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return target_fee_sat / Decimal(1e8) # Return result in  BRN
